...
for (const viewport of viewports) {
  ...
  page.on("response", (response) => {
    if (response.status() >= 400) problems.push(`[${viewport.name}] HTTP ${response.status()}: ${response.url().slice(0, 220)}`);
  });
  ...
  await page.goto(`${base}/dashboard/finance/profitability`, { waitUntil: "networkidle", timeout: 45_000 });
  await page.getByRole("heading", { name: "Profitability / Margin" }).waitFor();
