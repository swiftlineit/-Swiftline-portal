...
import { listClientLedger, openAuthenticatedFile, type CreditLedgerEntry } from "@/lib/creditBilling";
...
export default function ClientCreditLedgerPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const requestedId = new URLSearchParams(window.location.search).get("businessAccountId") || "";
        const selectedId = eligible.some((item) => item.account.id === requestedId)
          ? requestedId
          : eligible[0]?.account.id ?? "";
        if (!selectedId) throw new Error("No business account with financial access was found.");
        ...
        if (selectedId !== requestedId) {
          router.replace(`/client/credit/ledger?businessAccountId=${selectedId}`);
        }
      } catch (caught) {
        if (active) setError(caught instanceof Error ? caught.message : "Account statement could not be loaded.");
      } finally {
  ...
  async function changeAccount(accountId: string) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Account statement could not be loaded.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-slate-950">Account Statement</h1>
            <p className="mt-1 text-sm text-slate-600">Credit activity and resulting available balances.</p>
          </div>
          {businessAccountId ? (
            <button type="button" onClick={() => void openAuthenticatedFile(`/api/v1/client/credit/ledger/export?businessAccountId=${businessAccountId}`, "credit-account-statement.csv")} className="inline-flex h-10 items-center gap-2 border border-slate-300 bg-white px-4 text-sm font-semibold text-blue-900">
        ...
        <section className="overflow-x-auto border border-slate-200 bg-white">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-100 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Date</th><th className="px-4 py-3">Activity</th><th className="px-4 py-3">Reference</th><th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3 text-right">Available Credit</th><th className="px-4 py-3 text-right">Customer Advance</th></tr></thead>
            <tbody>{entries.map((entry) => <tr key={entry.id} className="border-t border-slate-100"><td className="whitespace-nowrap px-4 py-4 text-slate-600">{formatDashboardDateTime(entry.createdAt)}</td><td className="px-4 py-4"><p className="font-semibold text-slate-900">{label(entry.type)}</p><p className="mt-1 max-w-md text-xs text-slate-500">{entry.description}</p></td><td className="px-4 py-4 text-slate-700">{entry.reference}</td><td className="px-4 py-4 text-right font-semibold">{money(entry.amountMinor)}</td><td className="px-4 py-4 text-right">{money(entry.availableCreditAfterMinor)}</td><td className="px-4 py-4 text-right">{money(entry.availableAdvanceAfterMinor)}</td></tr>)}</tbody>
          </table>
          {loading ? <p className="p-10 text-center text-sm font-semibold text-slate-500"><FiRefreshCw className="mr-2 inline animate-spin" />Loading account statement...</p> : null}
          {!loading && !entries.length ? <p className="p-10 text-center text-sm text-slate-500">No credit activity has been recorded.</p> : null}
        </section>
      </div>
