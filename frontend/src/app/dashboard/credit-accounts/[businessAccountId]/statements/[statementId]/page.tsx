...
export default function AdminCreditStatementDetailPage() {
  ...
  const params = useParams<{ businessAccountId: string; statementId: string }>();
    ...
    <DashboardShell user={user}>
      <div className="mx-auto max-w-6xl space-y-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {statement ? <div className="flex gap-2"><button type="button" onClick={() => void openAuthenticatedFile(pdfPath)} className="h-10 border border-slate-300 px-4 text-sm font-semibold text-blue-900">View</button><button type="button" onClick={() => void openAuthenticatedFile(`${pdfPath}?download=1`, `${statement.statementNumber.replaceAll("/", "-")}.pdf`)} className="inline-flex h-10 items-center gap-2 border border-slate-300 px-4 text-sm font-semibold text-blue-900"><FiDownload /> Download</button><button type="button" onClick={() => void printAuthenticatedPdf(pdfPath)} className="inline-flex h-10 items-center gap-2 border border-slate-300 px-4 text-sm font-semibold text-blue-900"><FiPrinter /> Print</button></div> : null}
        </div>
        {error ? <div role="alert" className="border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div> : null}
        {loading ? <div className="p-10 text-center text-sm text-slate-500"><FiRefreshCw className="mr-2 inline animate-spin" />Loading statement...</div> : null}
        {statement ? <>
          <section className="grid border border-slate-200 bg-white sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div className="border-b border-slate-200 p-5 lg:border-b-0 lg:border-r"><p className="text-xs font-semibold uppercase text-slate-500">Total</p><p className="mt-2 font-semibold">{money(statement.totalAmountMinor)}</p></div>
