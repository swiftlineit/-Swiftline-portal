...
export const setOperationsParcelValue = (id: string, consignmentId: string, parcelNumber: string, valueMinor: number) => request(`/api/v1/operations-manifests/${id}/consignments/${consignmentId}/value`, { method: "PATCH", body: JSON.stringify({ parcelNumber, valueMinor }) });
...
const manifestExportPaths: Record<"xlsx" | "pdf" | "edi", { path: string; fileName: (manifestNumber: string) => string }> = {
  xlsx: { path: "export.xlsx", fileName: (number) => `ops-manifest-${number}.xlsx` },
  pdf: { path: "export.pdf", fileName: (number) => `ops-manifest-${number}.pdf` },
  edi: { path: "export-edi.xlsx", fileName: (number) => `edi-${number}.xlsx` }
};

export async function downloadOperationsManifest(id: string, format: "xlsx" | "pdf" | "edi", view = false, manifestNumber = "") {
  const token = getAccessToken() ?? await refreshAccessToken();
  const target = manifestExportPaths[format];
  const response = await fetch(apiUrl(`/api/v1/operations-manifests/${id}/${target.path}${view ? "?view=1" : ""}`), { headers: { Authorization: `Bearer ${token}` } });
  ...
  if (view) window.open(url, "_blank", "noopener,noreferrer"); else { const link = document.createElement("a"); link.href = url; link.download = target.fileName(manifestNumber || id); link.click(); }
