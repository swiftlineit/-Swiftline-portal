...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok || !data.success) throw new Error(data.message || "The operations manifest request could not be completed.");
  return data as T;
}
...
export const scanOperationsParcel = (id: string, bagId: string, parcelNumber: string, scanRequestId: string) => request<{ success: true } & ManifestDetail>(`/api/v1/operations-manifests/${id}/scan`, { method: "POST", body: JSON.stringify({ bagId, parcelNumber, scanRequestId }) });
