...
export default function AdminDashboardHeader({ user }: { user: AuthenticatedUser }) {
  ...
  return (
    <section className={`flex flex-col  gap-6 p-6 ${panelSurface}`}>
        ...
        {actions.map((action) => {
          ...
          return (
            <Link
              key={action.href}
              href={action.href}
              className={`inline-flex items-center gap-2 rounded-4xl px-4 py-2.5 text-sm font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0D1282]/40 focus-visible:ring-offset-2 ${
                action.primary
