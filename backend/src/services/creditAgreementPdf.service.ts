...
export function createCreditAgreementPdf(agreement: ICreditAgreement, generatedAt: Date, signature?: AgreementSignature) {
  ...
  y = drawClause(doc, 3, "Invoices and amendments", "Swiftline will issue an invoice for a booked shipment, with GST included where applicable. If an approved shipment amendment changes the charge, Swiftline may issue a new invoice revision while preserving earlier invoice versions in the shipment record.", y);
  ...
  y = drawClause(doc, 6, "Rates and final charges", "Booking estimates are calculated from the applicable rate card and GST treatment. Final charges may change after operational weight verification or an approved amendment. Any additional amount requires sufficient available booking capacity.", y);
